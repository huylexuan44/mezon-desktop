...
impl FieldPos {
        ...
        T::Error: std::fmt::Debug,
    {
        match self {
            ...
            Self { start, end } => {
                // mezon vendor edit: non-conformant bus implementations (ibus-daemon
                // sets its well-known name as the message sender) must not panic the
                // connection executor; treat an unparseable field as absent instead.
                let s = std::str::from_utf8(&msg_buf[(*start as usize)..(*end as usize)]).ok()?;
                T::try_from(s).ok()
            }
        }
    }
